...
function ChartViewer({ series, title }: ChartViewerProps) {
  ...
  const hiddenSeriesCount = series.length - Object.values(selectedSeries).filter(Boolean).length;
  ...
  const visibleSeries = useMemo(
    () => series.filter(s => selectedSeries[s.Id]),
    [series, selectedSeries]
  );

  // Use the longest series for labels - memoized
  const { sortedBaseData, labels } = useMemo(() => {
    const longest = series.reduce((prev, current) => (prev.Data.length > current.Data.length) ? prev : current);
    const sorted = [...longest.Data].sort((a, b) => a.Fecha - b.Fecha);
    const lbls = sorted.map(d => format(new Date(d.Fecha), 'MMM yy', { locale: es }));
    return { sortedBaseData: sorted, labels: lbls };
  }, [series]);

  // Memoize datasets to avoid recalculation on every render
  const datasets = useMemo(() => {
    return visibleSeries.map((serie, index) => {
  ...
  }, [visibleSeries, sortedBaseData, chartType]);
  ...
  const handleDownloadCSV = () => {
    ...
    // Optimized: Create Map for each series (O(n) instead of O(n²))
    const seriesMaps = series.map(s => ({
      nombre: s.Nombre,
      dataMap: new Map(s.Data.map(d => [d.Fecha, d.Valor]))
    }));
    ...
    const csvData = sortedDates.map(date => {
      ...
      seriesMaps.forEach(({ nombre, dataMap }) => {
        row[nombre] = dataMap.get(date) ?? '';
      });
      return row;
    });
        ...
        <div className="md:w-64 flex-shrink-0 space-y-4">
            <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-200 dark:border-gray-700 max-h-[400px] overflow-y-auto">
                    ...
                    <h3 className="font-semibold text-sm text-gray-700 dark:text-gray-300">
                      ...
                      {hiddenSeriesCount > 0 && (
                        <span className="ml-2 text-xs text-gray-500 dark:text-gray-400 font-normal">
                          ({hiddenSeriesCount} oculta{hiddenSeriesCount !== 1 ? 's' : ''})
                        </span>
                      )}
                    </h3>
                    <div className="flex gap-1">
                        ...
                        <span className="text-gray-300" aria-hidden="true">|</span>
                         <button
                            ...
                            className="text-xs text-blue-600 hover:underline"
                            aria-label="Ocultar todas las series"
                        >
                            Ninguna
                        </button>
                    </div>
                </div>
                <div className="space-y-1" role="group" aria-label="Selección de series">
                    {series.map(s => (
                        <div key={s.Id} className="flex items-start gap-2 text-sm group">
                             <button
                                ...
                                aria-label={`${selectedSeries[s.Id] ? 'Ocultar' : 'Mostrar'} serie ${s.Nombre}`}
                                aria-pressed={selectedSeries[s.Id]}
                             >
                                {selectedSeries[s.Id] && <Eye className="w-3 h-3" aria-hidden="true" />}
                             </button>
                             <span
                                onClick={() => toggleSerie(s.Id)}
             ...
             <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <div className="flex gap-2" role="group" aria-label="Tipo de gráfico">
                    <button
                ...
                <div className="h-4 w-px bg-gray-300 dark:bg-gray-600" aria-hidden="true"></div>
                 <div className="flex gap-1" role="group" aria-label="Exportar datos">
                    <button
                    ...
                    <button
                        ...
                        className="p-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                        aria-label="Descargar gráfico como imagen PNG"
                    >
...
export default memo(ChartViewer);
