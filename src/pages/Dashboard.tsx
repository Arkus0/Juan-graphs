...
const FavoriteCard = ({ item }: { item: any }) => {
  ...
  const { data: series, isLoading } = useQuery({
    queryKey: ['datos', item.tablaId],
    queryFn: () => getDatosTabla(item.tablaId, 6), // Last 6 periods for small chart
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-4">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 line-clamp-2 h-12" title={item.nombreTabla}>{item.nombreTabla}</h3>
            <Link to={`/ine/operacion/${item.operacionId}`} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
